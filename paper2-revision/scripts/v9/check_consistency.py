#!/usr/bin/env python3
"""v9 executable consistency check (extends v8; v9 registry in scripts/v9/format_numbers.py): numbers in the manuscript, Highlights and working-draft letter
...
_s = _ilu.spec_from_file_location("format_numbers_v9", Path(__file__).resolve().parent / "format_numbers.py")
FN = _ilu.module_from_spec(_s)
_s.loader.exec_module(FN)

REV = Path(__file__).resolve().parents[2]
OUTJ = REV / "results" / "v9" / "check_consistency_report.json"
OUTM = REV / "results" / "v9" / "check_consistency_report.md"
...
CTX_DOCS = {"tex": (REV / "paper2-csl_v9.tex").read_text(encoding="utf-8"),
            "letter": (REV / "response_letter_v9.md").read_text(encoding="utf-8")}
...
def main():
    ...
    # ---- B: tokens
    tex = (REV / "paper2-csl_v9.tex").read_text(encoding="utf-8")
    ...
    nb = (REV / "NUMBERS_v9.md").read_text(encoding="utf-8")
    for m in sorted(set(re.findall(r"`((?:results|corrected|scripts|frozen_package|v5_investigation)/[^`]+?)`", nb))):
        ...
        if not (REV / path).exists():
            fails.append(f"D NUMBERS_v9 names a missing file: {path}")
    counts = {}
    ...
    lines = ["# check_consistency report (v9)", "", f"Registry entries: {len(FN.E)}. Token status counts: {counts}.",
