#!/usr/bin/env python
"""Test 6 (v8): cross-backbone stability on the corrected inputs, with speaker metadata from the master.

v8 change (v7 check C-1): the five non-HuBERT-base files carry aetiology = "unknown" for the 1,045
SAP speakers restored by the join repair (and for one Hungarian_HC speaker). The analysis now takes
aetiology, is_control and severity_label for EVERY (dataset, speaker_id) key from the corrected
master (the canonical metadata table); each backbone file contributes only its d-prime values.
Counts of relabelled rows are written to the output. COPAS speakers with a non-dysarthric
aetiology are excluded from the severity bins, as in Figure 4.
...
CORR = BASE / "corrected"
OUT = BASE / "results" / "v8" / "v8_test6.json"
...
def load_meta():
    meta = {}
    for r in csv.DictReader(open(CORR / "track4_master.csv", encoding="utf-8")):
        meta[(r["dataset"], r["speaker_id"])] = r
    return meta
...
def load(fn, excl_nl=False):
    ...
    for r in csv.DictReader(open(CORR / fn, encoding="utf-8")):
        ...
        keys.append(key)
        m = META[key]
        for c in ("aetiology", "is_control", "severity_label"):
            if r[c] != m[c]:
                changed[f"{c}:{r['dataset']}:{r[c]}->{m[c]}"] += 1
        sev = "control" if m["is_control"].lower() == "true" else m["severity_label"]
        if m["dataset"] == "COPAS" and m["aetiology"] in NONDYS_COPAS and sev != "control":
            sev = None
        rows[key] = {"vec": np.array([fnum(r[c]) for c in FEATS]),
                     "aet": "HC" if m["aetiology"] == "healthy" else AETS.get(m["aetiology"]),
                     "hc": m["aetiology"] == "healthy",
                     "sev": sev if sev in SEV else None}
    if not excl_nl:
        RELABEL[fn] = dict(changed)
...
def main():
    ...
    for b, d in data.items():
        ...
        sev_rho[b] = {"rho": float(r.statistic), "p": float(r.pvalue), "n": len(xy)}

    out = {
        "script": "scripts/v8/v8_test6.py",
        ...
        "per_pair": {k: {"rho": v["rho"], "n": v["n"]} for k, v in sp.items()},
        "rho_min": min(rhos.values()), "min_pair": min(rhos, key=rhos.get),
        "rho_max": max(rhos.values()), "max_pair": max(rhos, key=rhos.get),
        "n_range": [min(ns), max(ns)],
        "dutch_exclusion": {"rho_min_with_nl": min(rhos.values()),
                            "rho_min_without_nl": min(rhos_nl.values()),
                            "min_pair_without_nl": min(rhos_nl, key=rhos_nl.get),
                            "per_pair_without_nl": dict(rhos_nl),
        ...
        "profile_cosine_min_per_aetiology": pc_min,
        "profile_cosine_min_pair_per_aetiology": {a: min(p, key=lambda k: p[k]["cos"]) for a, p in pc.items() if p},
        "profile_cosine_min_overall": min(pc_min.values()),
        "profile_cosines": {a: {k: {"cos": v["cos"], "ci95": v["ci95"],
                                    "n_spk": v["n_spk"]} for k, v in p.items()} for a, p in pc.items()},
        "severity_rho_per_backbone": sev_rho,
        "kendalls_w": kw,
        "kendalls_w_range": [min(kw.values()), max(kw.values())],
        "metadata_source": "corrected/track4_master.csv (aetiology, is_control, severity_label) for every key",
        "rows_relabelled_per_file": RELABEL,
        "group_n_per_backbone": {b: dict(Counter(v["aet"] for v in d.values() if v["aet"])) for b, d in data.items()},
