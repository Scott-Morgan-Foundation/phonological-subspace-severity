#!/usr/bin/env python3
"""v10 (from scripts/v8/v8_cite_order.py; reads the v10 manuscript): first-occurrence order of numeric citations [n] in the manuscript body (from the Introduction heading to the reference list; the abstract has no numeric citations). Writes results/v10/v10_cite_order.json with the order and every position where the
...
(REV / "results" / "v10" / "v10_cite_order.json").write_text(json.dumps(res, indent=1), encoding="utf-8")
