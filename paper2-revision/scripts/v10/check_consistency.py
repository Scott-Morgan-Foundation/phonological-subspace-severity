#!/usr/bin/env python3
"""v10 executable consistency check (extends v9; registry in scripts/v10/format_numbers.py): numbers in the manuscript,
Highlights and working-draft letter against the saved outputs. Exits non-zero on ANY failure. Do not weaken it to pass.
Runs from inside the verification package: every path is relative to the package root (two levels above this file).

Scope. Scanned: paper2-csl_v10.tex from \\begin{abstract} to the reference list; submission_highlights_v2.txt;
response_letter_v10.md. Every numeric token is scanned except single-digit integers (design descriptors
...
Checks.
 ...
 D. Every source named in NUMBERS_v10.md exists.
 H. High-risk regions (abstract, Highlights, contributions, every table, every figure caption, Conclusion): EVERY
    number there, single digits included, must be bound: scripts/v10/hr_bindings.json freezes, for each number, its
    surrounding text and the saved output it must equal (built by scripts/v10/build_hr_bindings.py from
    scripts/v10/hr_spec.py). The check re-derives each value from its saved output, requires the frozen context with
    that value to be present, and fails on any number in these regions that no binding covers. Only structural numbers
    (citation numbers, cross-references, layout widths, identifiers, cited years, the 95% level) are exempt.
    Outside these regions check B applies: a number passes if a saved output contains a value that rounds to it,
    which does not bind it to its sentence (see the planted-error test for the catch rate there).
Output: results/v10/check_consistency_report.json and .md.
"""
import csv
import json
import re
...
_s = _ilu.spec_from_file_location("format_numbers_v10", Path(__file__).resolve().parent / "format_numbers.py")
FN = _ilu.module_from_spec(_s)
_s.loader.exec_module(FN)

REV = Path(__file__).resolve().parents[2]
OUTJ = REV / "results" / "v10" / "check_consistency_report.json"
OUTM = REV / "results" / "v10" / "check_consistency_report.md"
TEX_NAME, LETTER_NAME, NUMBERS_NAME = "paper2-csl_v10.tex", "response_letter_v10.md", "NUMBERS_v10.md"
...
SOURCES = [
    ...
    "results/v5/fig2_group_means.json", "results/v10/fig4_adjacent_intervals_v10.json",
    ...
    "results/v9/v9_originals.json",
    "results/v9/originals/corrected/robustness_analyses.log",
    "results/v9/originals/corrected/fixed_token_dprime.log",
    "results/v9/originals/corrected/robustness_pass5.log",
    "results/v10/v10_facts.json", "results/v10/v10_hr_values.json", "results/v10/v10_fixed_token_groups.json",
    "results/v10/v10_originals_reproduction.json", "results/v10/v10_cite_order.json",
]
TEXT_SOURCES = {"external/paper1_arxiv_manuscript.tex": "companion paper [2] (arXiv source copy)",
...
CTX_DOCS = {"tex": (REV / TEX_NAME).read_text(encoding="utf-8"),
            "letter": (REV / LETTER_NAME).read_text(encoding="utf-8")}
...
def main():
    ...
    # ---- A: registry
    F = FN.bind()
    ids = [e["id"] for e in FN.E]
    for d in sorted({i for i in ids if ids.count(i) > 1}):
        fails.append(f"A duplicate registry id {d}")
    for what, ok, detail in FN.claims(F):
        report.setdefault("claims", []).append({"claim": what, "ok": ok, "detail": detail})
        if not ok:
            fails.append(f"E claim does not hold: {what} ({detail})")
    printed = {}
    for e in FN.E:
        p = REV / e["src"]
        if not p.exists():
        ...
        if val.startswith("MISSING"):
            fails.append(f"A value missing in source for {e['id']}: {val}")
            continue
    ...
    # ---- B: tokens
    tex = (REV / TEX_NAME).read_text(encoding="utf-8")
    ...
    nb = (REV / NUMBERS_NAME).read_text(encoding="utf-8")
    for m in sorted(set(re.findall(r"`((?:results|corrected|scripts|frozen_package|v5_investigation)/[^`]+?)`", nb))):
        ...
        if not (REV / path).exists():
            fails.append(f"D {NUMBERS_NAME} names a missing file: {path}")
    counts = {}
    ...
    hr_fails, hr_report = check_high_risk(tex)
    fails += hr_fails
    report["high_risk"] = hr_report
    report["failures"] = fails
    OUTJ.write_text(json.dumps(report, indent=1, default=str), encoding="utf-8")
    lines = ["# check_consistency report (v10)", "", f"Registry entries: {len(FN.E)}. Token status counts: {counts}.",
             f"High-risk regions: {hr_report['n_tokens']} numbers, {hr_report['n_bound']} bound, {hr_report['n_structural']} structural, "
             f"{hr_report['n_bindings']} frozen bindings.",
             "Scope outside the high-risk regions: single-digit integers are not scanned.", "", f"## Failures ({len(fails)})", ""]
...
def check_high_risk(tex):
    """Check H (see module docstring)."""
    import sys as _sys
    here = Path(__file__).resolve().parent
    _sys.path.insert(0, str(here))
    import hr_regions as H
    from hr_resolve import resolve
    fails, rep = [], {"bindings": [], "unbound": []}
    F = FN.bind()
    hl = (REV / "submission_highlights_v2.txt").read_text(encoding="utf-8")
    R = H.regions(tex)
    R["highlights"] = (0, len(hl))
    B = json.loads((here / "hr_bindings.json").read_text(encoding="utf-8"))["bindings"]
    spans = {r: [] for r in R}
    for bnd in B:
        full = hl if bnd["region"] == "highlights" else tex
        a, b = R[bnd["region"]]
        try:
            val, _src = resolve(bnd["ref"], F, FN.fmt)
        except Exception as exc:          # a missing source or path is a failure, never a pass
            fails.append(f"H value not derivable for {bnd['ref']}: {exc!r}")
            continue
        if val.startswith(("NOT<", "NOT>", "MISSING")):
            fails.append(f"H saved value does not support the printed bound for {bnd['ref']}: {val}")
        rendered = bnd["before"] + val + bnd["after"]
        found, j = [], full.find(rendered, a)
        while 0 <= j and j + len(rendered) <= b:
            found.append(j)
            j = full.find(rendered, j + 1)
        ok = bool(found)
        if not ok:
            fails.append(f"H {bnd['region']}: context with saved value not found ({bnd['ref']} = {val!r}): "
                         f"{(bnd['before'][-25:] + '[' + val + ']' + bnd['after'][:12])!r}")
        for j in found:
            s0 = j + len(bnd["before"])
            spans[bnd["region"]].append((s0, s0 + len(val)))
        rep["bindings"].append({"region": bnd["region"], "ref": bnd["ref"], "value": val, "found": ok})
    n_tok = n_bound = n_struct = 0
    for region, (a, b) in R.items():
        full = hl if region == "highlights" else tex
        for t in H.tokens(full[a:b], a):
            n_tok += 1
            if H.structural(full, t):
                n_struct += 1
                continue
            if any(s < t["e"] and t["s"] < e for s, e in spans[region]):
                n_bound += 1
                continue
            ctx = full[max(0, t["s"] - 40):t["e"] + 20].replace("\n", " ")
            fails.append(f"H {region}: number not bound to a saved output: {t['tok']!r} ... {ctx!r}")
            rep["unbound"].append({"region": region, "tok": t["tok"], "ctx": ctx})
    rep.update({"n_tokens": n_tok, "n_bound": n_bound, "n_structural": n_struct, "n_bindings": len(B)})
    return fails, rep
