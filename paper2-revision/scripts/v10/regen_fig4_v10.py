#!/usr/bin/env python3
"""Figure 4 (severity gradient across 6 SSL backbones), Paper 2 v10 (from regen_fig4_v5.py; Bernard's decision D3 = option D).

v10 change: speaker metadata (severity_label, is_control, aetiology) comes from the corrected master for every
panel, as the Methods state; each backbone file contributes only its d-prime values. v5 read each backbone file's
own labels, which differ from the master for about 124 keys (v9_check F-4b).
...
saved to results/v10/fig4_adjacent_intervals_v10.json (these are the numbers the caption relies on).
...
CORR = BASE / "corrected"
OUT_PDF = BASE / "figures" / "fig4_severity_v10.pdf"
OUT_PNG = BASE / "figures" / "fig4_severity_v10.png"
OUT_JSON = BASE / "results" / "v10" / "fig4_adjacent_intervals_v10.json"
...
META = {(r["dataset"], r["speaker_id"]): r for r in csv.DictReader(open(CORR / "track4_master.csv", encoding="utf-8"))}
META_COLS = ("severity_label", "is_control", "aetiology")
...
def load_groups(fn):
    ...
    missing_meta = 0
    for r in rows:
        m = META.get((r["dataset"], r["speaker_id"]))
        if m is None:
            missing_meta += 1
            continue
        for c in META_COLS:
            r[c] = m[c]
    assert missing_meta == 0, (fn, missing_meta)
...
OUT_JSON.write_text(json.dumps({
    "script": "scripts/v10/regen_fig4_v10.py", "metadata_source": "corrected/track4_master.csv for every panel", "inputs": "corrected/", "seed": SEED, "n_boot": N_BOOT,
